...
export default function AgeScrollPicker({ value, min, max, onChange }: AgeScrollPickerProps) {
  ...
  return (
    <div className="relative h-[132px] w-full overflow-hidden bg-white rounded-xl border border-[#EAEAEA] shadow-inner select-none touch-pan-y">
      ...
      >
        ...
        {options.map((opt) => {
          ...
          return (
            <div 
              key={opt}
              className={`h-[44px] shrink-0 flex items-center justify-center snap-center w-full transition-all duration-150 ${
                isSelected ? 'text-[#D56B45] font-black text-2xl scale-110' : 'text-[#767676] font-medium text-lg'
              }`}
            >
              {opt}
            </div>
