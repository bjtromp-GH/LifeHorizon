...
export default function OnboardingIntro({ initialStep = 0, inputs, hasStoredData, onInputChange, onComplete }: OnboardingIntroProps) {
  ...
  return (
    <div className="h-[100dvh] md:h-auto md:min-h-screen overflow-hidden bg-[#FDFDFD] flex flex-col justify-between p-3.5 pt-safe-sm sm:p-8 sm:pt-safe font-sans text-[#2D2D2D] select-none relative">
      ...
      <main ref={mainScrollRef} className={`flex-grow flex flex-col items-center pt-6 sm:pt-10 z-10 w-full max-w-xl mx-auto px-1 min-h-0 ${step === 2 || step === 3 || step === 7 ? 'pb-6 sm:pb-10' : 'pb-24 sm:pb-32'} ${step <= 1 ? 'overflow-hidden' : 'overflow-y-auto'}`}>
        <AnimatePresence 
          mode="wait" 
          onExitComplete={() => {
            if (mainScrollRef.current) {
        ...
        >
          {isTransitioning ? (
            ...
            >
              ...
              <div className="flex-grow flex flex-col items-center justify-center text-center max-w-xl mx-auto px-4 z-10 space-y-4 sm:space-y-7">
                ...
                <motion.div 
                  ...
                  initial="hidden"
                  animate="visible"
                  className="flex justify-center items-center flex-nowrap w-full"
                >
                    ...
                    <span key={wordIndex} className="flex whitespace-nowrap">
                      {Array.from(word).map((char, charIndex) => (
                        <motion.span
                          key={charIndex}
                          variants={letterVars}
                          className={`text-[26px] min-[375px]:text-[32px] min-[415px]:text-[36px] sm:text-6xl font-sans font-black tracking-tight select-none inline-block text-white ${wordIndex > 0 && charIndex === 0 ? "ml-1.5 sm:ml-3" : ""}`}
                          style={{
            ...
            >
              ...
              <div className="flex-grow flex flex-col items-center justify-center max-w-lg mx-auto z-10 space-y-4 sm:space-y-6 w-full mt-6 sm:mt-8 text-center">
                <motion.p 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div
              ...
              transition={{ duration: 0.4 }}
              className="text-center space-y-4 sm:space-y-6 max-w-md w-full px-1 relative my-auto py-4"
            >
              <Confetti />
